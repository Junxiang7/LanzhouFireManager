...
{
    private static string m_DBConnectionString = ConfigurationManager.ConnectionStrings["MySqlConnString"].ConnectionString;
    ...
    //protected static MySqlConnection GetMySqlHelper()
    //{
    //    MySqlConnection MysqlDBConnectionString = new MySqlConnection(m_DBConnectionString);
    ...
    {
        return ExecuteScalar(DBConnectionString, CommandType.Text, commandText, parms);
    }
    ...
    {
        return ExecuteDataTable(DBConnectionString, CommandType.Text, commandText, parms);
    }
    ...
    private static void PrepareCommand(MySqlCommand command, MySqlConnection connection, MySqlTransaction transaction, CommandType commandType, string commandText, MySqlParameter[] parms)
    {
        ...
        // 设置命令文本(存储过程名或SQL语句)
        command.CommandText = commandText.Replace("char(", " ").Replace("CHAR(", " "); ;
        // 分配事务
        if (transaction != null)
        {
    ...
    /// <param name="parms">查询参数</param>
    /// <returns>返回影响的行数</returns>
    public static int ExecuteNonQuery(string commandText, CommandType ComType, params MySqlParameter[] parms)
    {
        return ExecuteNonQuery(DBConnectionString, ComType, commandText, parms);
    }
    ...
    {
        return ExecuteScalar(DBConnectionString, commandType, commandText, parms);
    }
    ...
    /// <param name="parms">查询参数</param>
    /// <returns>返回只读数据集</returns>
    public static MySqlDataReader ExecuteDataReader(string commandText, CommandType commandType, params MySqlParameter[] parms)
    {
        return ExecuteDataReader(DBConnectionString, commandType, commandText, parms);
    }
    ...
    {
        return ExecuteDataSet(DBConnectionString, commandType, commandText, parms);
    }
